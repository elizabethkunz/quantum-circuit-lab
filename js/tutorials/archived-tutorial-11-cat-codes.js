/* =========================================================================
   TUTORIAL 11: CAT CODES (BOSONIC ENCODING) — ARCHIVED / NOT LOADED
   Markup lives in index.html inside <template id="archived-tutorial-11-cat-codes">.
   The Learn subtab and this script include are commented out. Restore per notes there
   and in learn-core.js (switchSubtab list + progress totals).

   1D Gaussian line model (pedagogical stand-in for one quadrature in phase
   space). Not a full continuous-variable simulation.
   =========================================================================

   HTML ids (prefix t11archive- in template): mot-*, coh-*, cat-*, loss-*, parity-*,
   step-next t11archive-1-next … t11archive-5-next; data-step t11archive-1 … 6.
   ========================================================================= */
...
(function tutorial11() {
  ...
  (function step1() {
    ...
    const loss = document.getElementById('t11archive-mot-loss');
    const cv = document.getElementById('t11archive-mot-cv');
    const next = document.getElementById('t11archive-1-next');
    const hint = document.getElementById('t11archive-mot-hint');
    if (!loss || !cv) return;
  ...
  /* --- Step 2 --- */
  (function step2() {
    const slider = document.getElementById('t11archive-coh-alpha');
    const valEl = document.getElementById('t11archive-coh-alpha-val');
    const cap = document.getElementById('t11archive-coh-caption');
    const next = document.getElementById('t11archive-2-next');
    if (!slider) return;
    ...
    function draw() {
      ...
      drawLinePlot('t11archive-coh-plot', [
        { y: p1 },
        { y: p2 }
      ], ['|ψ_α|²', '|ψ_−α|²']);
      if (cap) {
  ...
  /* --- Step 3 --- */
  (function step3() {
    const bEven = document.getElementById('t11archive-cat-b-even');
    const bOdd = document.getElementById('t11archive-cat-b-odd');
    const slider = document.getElementById('t11archive-cat-alpha');
    const valEl = document.getElementById('t11archive-cat-alpha-val');
    const ro = document.getElementById('t11archive-cat-readout');
    const cap = document.getElementById('t11archive-cat-caption');
    const next = document.getElementById('t11archive-3-next');
    if (!slider) return;
    let mode = 'even';
    ...
    function draw() {
      ...
      const ov = inner(p, m);
      drawLinePlot('t11archive-cat-plot', [{ y: pdf }], [mode === 'even' ? 'even |cat+⟩' : 'odd |cat−⟩']);
      if (ro) {
  ...
  /* --- Step 4 --- */
  (function step4() {
    const lossS = document.getElementById('t11archive-loss');
    const lossVal = document.getElementById('t11archive-loss-val');
    const aS = document.getElementById('t11archive-loss-alpha');
    const aVal = document.getElementById('t11archive-loss-alpha-val');
    const out = document.getElementById('t11archive-loss-readout');
    const next = document.getElementById('t11archive-4-next');
    if (!lossS) return;
    ...
    function draw() {
      ...
      drawLinePlot('t11archive-loss-plot', [
        { y: pure },
        { y: mix }
  ...
  /* --- Step 5 --- */
  (function step5() {
    const truth = document.getElementById('t11archive-parity-truth');
    const lossS = document.getElementById('t11archive-parity-loss');
    const lossVal = document.getElementById('t11archive-parity-loss-val');
    const aS = document.getElementById('t11archive-parity-alpha');
    const aVal = document.getElementById('t11archive-parity-alpha-val');
    const pred = document.getElementById('t11archive-parity-pred');
    const btn = document.getElementById('t11archive-parity-btn');
    const log = document.getElementById('t11archive-parity-log');
    const next = document.getElementById('t11archive-5-next');
    if (!btn) return;
